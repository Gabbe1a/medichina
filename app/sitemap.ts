...
export default async function sitemap(): Promise<MetadataRoute.Sitemap> {
  const base = process.env.NEXT_PUBLIC_SITE_URL ?? "https://superdentos.ru";
  const [services, doctors] = await Promise.all([
    prisma.service.findMany({ where: { published: true } }),
    prisma.doctor.findMany({ where: { published: true } }),
  ]);

  const staticRoutes = ["", "/about", "/services", "/doctors", "/prices", "/reviews", "/contacts", "/privacy", "/legal", "/patients/faq"];
